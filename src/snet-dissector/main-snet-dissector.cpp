...
int main(int argc, char* argv[]) {
    try {
        ...
        if (l_VariablesMap.count("help")) {
            ...
            std::cout << "The dissector for s-net(r) packets for the HDLC Daemon is Copyright (C) 2016, and GNU GPL'd, by Florian Evers." << std::endl;
        ...
        if (boost::regex_match(l_VariablesMap["connect"].as<std::string>(), l_Match, s_RegEx)) {
            ...
            // Prepare HDLCd access protocol entity: 0x23 = Payload Raw RO, RX and TX, RECV_CTRL
            HdlcdClient l_HdlcdClient(l_IoService, l_EndpointIterator, l_Match[1], 0x23);
